...
export const FUNNEL_SQL_PROMPT = [
  ...
  "",
  "## ⚠️ CRITICAL: SQL GROUP BY / ORDER BY Correctness",
  "",
  ...
  "- `ORDER BY CASE(same_expr)` where CASE matches a grouped expression",
  "- `ORDER BY sort_order` when `sort_order` is in both `SELECT` and `GROUP BY`",
  "",
  ...
  "- `ORDER BY different_column` — Column must be in GROUP BY or aggregated",
  "- `COUNT(MAX(column))` — Nested aggregates are invalid",
  "- `PERCENTILE_CONT(...) OVER (PARTITION BY x)` with `GROUP BY x` — invalid in SQL Server",
  "",
  "### ✅ Correct Pattern for Median by Group:",
  "- Use `SELECT DISTINCT` with windowed percentile (no GROUP BY)",
  "- Example:",
  "  SELECT DISTINCT Stage,",
  "    CAST(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY TimeToHealingInDays)",
  "      OVER (PARTITION BY Stage) AS DECIMAL(10,2)) AS MedianDays",
  "  FROM SourceCTE",
  "  ORDER BY Stage",
  "",
  "### ✅ Correct Pattern for Categorical Aggregations:",
  "When using GROUP BY with CASE, add an explicit sort column:",
  "- SELECT: include CASE expression, 1 AS SortOrder, COUNT(*) AS ItemCount",
  "- GROUP BY: include the same CASE expression, and 1",
  "- ORDER BY: use SortOrder (not the CASE or ungrouped columns)",
  "",
  "Pattern:",
  "  SELECT CASE ... END AS CategoryName, 1 AS SortOrder, COUNT(*) AS ItemCount",
  "  FROM table",
  "  GROUP BY CASE ... END, 1",
  "  ORDER BY SortOrder",
  "",
  "**Validation Checklist:**",
  "- [ ] Every column in ORDER BY is either in GROUP BY or an aggregate",
  "- [ ] CASE expressions in ORDER BY match grouped expressions",
  "- [ ] No nested aggregates",
  "- [ ] All non-aggregate columns in ORDER BY appear in GROUP BY",
  "",
