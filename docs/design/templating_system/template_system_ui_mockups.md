# Template System UI Mockups

**Last Updated:** 2025-10-06  
**Related:** [template_improvement_design.md](./template_improvement_design.md), [template_system_mvp_implementation_plan.md](../todos/template_system_mvp_implementation_plan.md)

## Purpose

This document provides detailed UI/UX mockups for the Template System (Phase 1 MVP). These mockups are designed to match the existing shadcn/ui + Radix + Tailwind design system used in the funnel UI.

**Target Users:** Developers creating and managing AI query templates

---

## Table of Contents

1. [Template Editor Modal (Create/Edit)](#template-editor-modal-createedit)
2. [Template Admin Page (Browse/Manage)](#template-admin-page-browsemanage)
3. [Apply Template Wizard (Slot-Filling)](#apply-template-wizard-slot-filling)
4. [Funnel Panel Template Info](#funnel-panel-template-info)
5. [Field Mapping Reference](#field-mapping-reference)
6. [Component Usage Guidelines](#component-usage-guidelines)

---

## Template Editor Modal (Create/Edit)

**Path:** Accessed from:

- Funnel Panel: "Save as Template" button (create mode, pre-filled with SQL)
- Template Admin: "Edit" button on Draft templates
- Template Admin: "New Template" button (blank slate)

**Component:** Full-screen modal with tabs (`Dialog` + `Tabs` from shadcn/ui)

### Tab 1: Basic Info

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Create New Template                                                          ✕ │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                   │
│  ┌─ Tabs ────────────────────────────────────────────────────────────────────┐  │
│  │  [1. Basic Info] [2. SQL Pattern] [3. Placeholders] [4. Examples] Preview │  │
│  └───────────────────────────────────────────────────────────────────────────┘  │
│                                                                                   │
│  ╔═══════════════════════════════════════════════════════════════════════════╗  │
│  ║  TAB 1: BASIC INFO                                                        ║  │
│  ╠═══════════════════════════════════════════════════════════════════════════╣  │
│  ║                                                                           ║  │
│  ║  Template Name *                                                          ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │ Count Assessments by Time Window                                    │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║  💡 Use descriptive names that explain the query purpose                 ║  │
│  ║                                                                           ║  │
│  ║  Intent (Query Family) *                                                  ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │ Aggregation by category                                          ▼  │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║  Options:                                                                 ║  │
│  ║    • Aggregation by category                                             ║  │
│  ║    • Time-series trend                                                   ║  │
│  ║    • Top-k by metric                                                     ║  │
│  ║    • Join analysis                                                       ║  │
│  ║    • As-of state query                                                   ║  │
│  ║    • Pivot/unpivot                                                       ║  │
│  ║    • Cohort comparison                                                   ║  │
│  ║                                                                           ║  │
│  ║  Description                                                              ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │ Counts distinct assessments within a specified time window         │ ║  │
│  ║  │ for a given patient. Useful for analyzing patient visit            │ ║  │
│  ║  │ frequency and engagement over custom time periods.                 │ ║  │
│  ║  │                                                                     │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║  📝 Describe when this template should be used (100-300 chars)           ║  │
│  ║                                                                           ║  │
│  ║  Keywords (comma-separated) *                                             ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │ count, assessments, time window, date range, patient               │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║  Detected: [count] [assessments] [time window] [date range] [patient]    ║  │
│  ║                                                                           ║  │
│  ║  Tags (for organization)                                                  ║  │
│  ║  ┌──────────────────────────────────┐                                    ║  │
│  ║  │ [assessment-analysis ✕] [time-series ✕] [+ Add tag]                 │ ║  │
│  ║  └──────────────────────────────────┘                                    ║  │
│  ║  Popular: [patient-analysis] [wound-tracking] [metrics]                  ║  │
│  ║                                                                           ║  │
│  ║  SQL Dialect                                                              ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │ MS SQL Server (default)                                          ▼  │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ╚═══════════════════════════════════════════════════════════════════════════╝  │
│                                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────────────┐│
│  │                                   [Cancel] [Save Draft] [Next: SQL Pattern →]││
│  └─────────────────────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────────────────┘
```

**Components Used:**

- `Dialog` (full-screen variant)
- `Tabs`, `TabsList`, `TabsTrigger`, `TabsContent`
- `Input` for name and keywords
- `Select` for intent and dialect
- `Textarea` for description
- `Badge` for tag display with remove button
- `Button` for actions

**Validation:**

- Name: Required, 3-100 chars
- Intent: Required, from taxonomy
- Keywords: Required, at least 3 keywords
- Description: Optional, recommended 100-300 chars

---

### Tab 2: SQL Pattern Editor

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Create New Template                                                          ✕ │
├─────────────────────────────────────────────────────────────────────────────────┤
│  ┌─ Tabs ────────────────────────────────────────────────────────────────────┐  │
│  │  [1. Basic Info] [2. SQL Pattern] [3. Placeholders] [4. Examples] Preview │  │
│  └───────────────────────────────────────────────────────────────────────────┘  │
│                                                                                   │
│  ╔═══════════════════════════════════════════════════════════════════════════╗  │
│  ║  TAB 2: SQL PATTERN                                                       ║  │
│  ╠═══════════════════════════════════════════════════════════════════════════╣  │
│  ║                                                                           ║  │
│  ║  SQL Pattern (use {placeholders} for dynamic values) *                    ║  │
│  ║  ┌─ Editor ──────────────────────────────────────────────────────────────┐║  │
│  ║  │  1  SELECT                                                            │║  │
│  ║  │  2      COUNT(DISTINCT A.id) AS assessmentCount                       │║  │
│  ║  │  3  FROM rpt.Assessment A                                             │║  │
│  ║  │  4  JOIN rpt.DimDate D ON A.dimDateFk = D.id                          │║  │
│  ║  │  5  WHERE                                                              │║  │
│  ║  │  6      A.patientFk = {patientId}                                     │║  │
│  ║  │  7      AND D.date > DATEADD(day, -{windowDays}, {endDate})          │║  │
│  ║  │  8      AND D.date <= {endDate}                                       │║  │
│  ║  │                                                                        │║  │
│  ║  │  [Format SQL] [Copy] [Paste from clipboard]                           │║  │
│  ║  └────────────────────────────────────────────────────────────────────────┘║  │
│  ║                                                                           ║  │
│  ║  ✨ Smart Actions                                                          ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  📋 [Copy from Similar Template]     🎯 [Load from Query History]   │ ║  │
│  ║  │  🔄 [Import from Clipboard]          📝 [Use Starter Template]      │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  🔍 Detected Placeholders                                                 ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  ✅ {patientId}  (line 6)                                            │ ║  │
│  ║  │  ✅ {windowDays}  (line 7)                                           │ ║  │
│  ║  │  ✅ {endDate}  (lines 7, 8)                                          │ ║  │
│  ║  │                                                                      │ ║  │
│  ║  │  💡 All placeholders detected! Configure them in the next tab.       │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  ⚡ Real-time Validation                                                  ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  ✅ SELECT-only check passed                                         │ ║  │
│  ║  │  ✅ Schema prefixes correct (rpt.)                                   │ ║  │
│  ║  │  ⚠️  Suggestion: Add TOP 1000 to limit result set                   │ ║  │
│  ║  │  ✅ No dangerous keywords detected                                   │ ║  │
│  ║  │  ✅ Valid MS SQL Server syntax                                       │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  📊 Schema Helper (click to insert)                                       ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  Common Tables:                                                      │ ║  │
│  ║  │    [rpt.Assessment] [rpt.Patient] [rpt.Wound] [rpt.WoundState]      │ ║  │
│  ║  │    [rpt.DimDate] [rpt.Note]                                          │ ║  │
│  ║  │                                                                      │ ║  │
│  ║  │  Common Patterns:                                                    │ ║  │
│  ║  │    [JOIN DimDate] [WHERE patient filter] [DATEADD time window]      │ ║  │
│  ║  │    [ROW_NUMBER() OVER] [COUNT DISTINCT] [GROUP BY]                  │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ╚═══════════════════════════════════════════════════════════════════════════╝  │
│                                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────────────┐│
│  │                     [← Back] [Save Draft] [Next: Placeholders →]            ││
│  └─────────────────────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────────────────┘
```

**Components Used:**

- `Textarea` with monospace font for SQL editor (consider Monaco/CodeMirror integration)
- `Alert` for validation messages (success/warning/error variants)
- `Button` for smart actions and helpers
- `Badge` for placeholder chips

**Validation:**

- SQL Pattern: Required, must contain at least one SELECT
- Placeholder detection: Regex `\{(\w+)\}` to find all placeholders
- Real-time checks: SELECT-only, schema prefix, dangerous keywords
- Line numbers for error reporting

**Smart Actions:**

- **Copy from Similar Template**: Opens dropdown of templates with similar intent
- **Load from Query History**: Browses recent successful queries in Funnel
- **Import from Clipboard**: Pastes SQL and attempts to detect placeholders
- **Use Starter Template**: Library of common SQL patterns (aggregation, join, etc.)

---

### Tab 3: Placeholders Configuration (Enhanced UX)

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Create New Template                                                          ✕ │
├─────────────────────────────────────────────────────────────────────────────────┤
│  ┌─ Tabs ────────────────────────────────────────────────────────────────────┐  │
│  │  [1. Basic Info] [2. SQL Pattern] [3. Placeholders] [4. Examples] Preview │  │
│  └───────────────────────────────────────────────────────────────────────────┘  │
│                                                                                   │
│  ╔═══════════════════════════════════════════════════════════════════════════╗  │
│  ║  TAB 3: PLACEHOLDERS CONFIGURATION                                        ║  │
│  ╠═══════════════════════════════════════════════════════════════════════════╣  │
│  ║                                                                           ║  │
│  ║  🎯 Quick Setup Presets                                                    ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  📋 [Patient Filter]  📅 [Time Window]  🔢 [Metric Aggregation]     │ ║  │
│  ║  │  🔍 [Top-K Limiter]   📊 [Dimension Grouping]  ⚙️ [Copy Template]   │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║  💡 Presets automatically configure common placeholder patterns           ║  │
│  ║                                                                           ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  Placeholder 1 of 3                                        [▲] [▼] ✕ │ ║  │
│  ║  ├─────────────────────────────────────────────────────────────────────┤ ║  │
│  ║  │                                                                      │ ║  │
│  ║  │  Placeholder Name: {patientId}  (detected from SQL)                 │ ║  │
│  ║  │                                                                      │ ║  │
│  ║  │  🏷️ Configuration Style:                                             │ ║  │
│  ║  │  ○ Simple Mode (basic types only)                                   │ ║  │
│  ║  │  ● Advanced Mode (full schema integration) ← Recommended            │ ║  │
│  ║  │                                                                      │ ║  │
│  ║  │  ┌─ Advanced Mode ─────────────────────────────────────────────┐   │ ║  │
│  ║  │  │                                                              │   │ ║  │
│  ║  │  │  Data Type *                                                 │   │ ║  │
│  ║  │  │  ┌──────────────────────────────────────────────────────┐   │   │ ║  │
│  ║  │  │  │ guid                                              ▼  │   │   │ ║  │
│  ║  │  │  └──────────────────────────────────────────────────────┘   │   │ ║  │
│  ║  │  │  Options: guid, int, string, date, boolean, decimal         │   │ ║  │
│  ║  │  │                                                              │   │ ║  │
│  ║  │  │  Semantic Type (enables schema hints) *                      │   │ ║  │
│  ║  │  │  ┌──────────────────────────────────────────────────────┐   │   │ ║  │
│  ║  │  │  │ patient_id                                        ▼  │   │   │ ║  │
│  ║  │  │  └──────────────────────────────────────────────────────┘   │   │ ║  │
│  ║  │  │  💡 Links to rpt.Patient → Shows patient search in UI       │   │ ║  │
│  ║  │  │                                                              │   │ ║  │
│  ║  │  │  Common semantic types:                                      │   │ ║  │
│  ║  │  │  • patient_id, wound_id, assessment_id (entity IDs)          │   │ ║  │
│  ║  │  │  • time_window, date_range_start, date_range_end (time)      │   │ ║  │
│  ║  │  │  • metric_name, dimension_name (analysis)                    │   │ ║  │
│  ║  │  │  • top_k_limit (result limiting)                             │   │ ║  │
│  ║  │  │  • none (generic, no schema hints)                           │   │ ║  │
│  ║  │  │                                                              │   │ ║  │
│  ║  │  │  ┌─ Schema Integration ─────────────────────────────────┐   │   │ ║  │
│  ║  │  │  │ Source Table: rpt.Patient                            │   │   │ ║  │
│  ║  │  │  │ Value Column: id                                     │   │   │ ║  │
│  ║  │  │  │ Display Column: firstName + ' ' + lastName           │   │   │ ║  │
│  ║  │  │  │ Searchable: [✓]  Filterable: [✓]                    │   │   │ ║  │
│  ║  │  │  │                                                       │   │   │ ║  │
│  ║  │  │  │ 🎨 UI Preview when applying template:                │   │   │ ║  │
│  ║  │  │  │ ┌────────────────────────────────────────────┐       │   │   │ ║  │
│  ║  │  │  │ │ patientId *                                │       │   │   │ ║  │
│  ║  │  │  │ │ ┌──────────────────────────────────────┐   │       │   │   │ ║  │
│  ║  │  │  │ │ │ 🔍 Search patients...            ▼  │   │       │   │   │ ║  │
│  ║  │  │  │ │ └──────────────────────────────────────┘   │       │   │   │ ║  │
│  ║  │  │  │ │ Recent: John Smith, Jane Doe, Bob Martin   │       │   │   │ ║  │
│  ║  │  │  │ └────────────────────────────────────────────┘       │   │   │ ║  │
│  ║  │  │  └──────────────────────────────────────────────────────┘   │   │ ║  │
│  ║  │  │                                                              │   │ ║  │
│  ║  │  │  Required: [✓]                                               │   │ ║  │
│  ║  │  │                                                              │   │ ║  │
│  ║  │  │  Default Value (optional)                                    │   │ ║  │
│  ║  │  │  ┌──────────────────────────────────────────────────────┐   │   │ ║  │
│  ║  │  │  │ (none - will prompt user)                            │   │   │ ║  │
│  ║  │  │  └──────────────────────────────────────────────────────┘   │   │ ║  │
│  ║  │  │  ℹ️ Leave empty to always prompt user when applying          │   │ ║  │
│  ║  │  │                                                              │   │ ║  │
│  ║  │  │  Validators (add rules)                                      │   │ ║  │
│  ║  │  │  ┌──────────────────────────────────────────────────────┐   │   │ ║  │
│  ║  │  │  │  [non-empty ✕]                                       │   │   │ ║  │
│  ║  │  │  │  [+ Add validator ▼]                                 │   │   │ ║  │
│  ║  │  │  │     • non-empty                                      │   │   │ ║  │
│  ║  │  │  │     • min: N (for numbers/strings)                   │   │   │ ║  │
│  ║  │  │  │     • max: N (for numbers/strings)                   │   │   │ ║  │
│  ║  │  │  │     • regex: pattern (for strings)                   │   │   │ ║  │
│  ║  │  │  │     • valid-guid, valid-date                         │   │   │ ║  │
│  ║  │  │  └──────────────────────────────────────────────────────┘   │   │ ║  │
│  ║  │  │                                                              │   │ ║  │
│  ║  │  └──────────────────────────────────────────────────────────────┘   │ ║  │
│  ║  │                                                                      │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  Placeholder 2 of 3: {windowDays}                      [▲] [▼] ✕    │ ║  │
│  ║  ├─────────────────────────────────────────────────────────────────────┤ ║  │
│  ║  │  ● Simple Mode ○ Advanced Mode                                       │ ║  │
│  ║  │  Type: [int ▼]  Semantic: [time_window ▼]  Required: [✓]            │ ║  │
│  ║  │  Default: [180]  Validators: [min:1 ✕] [max:365 ✕]                  │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  Placeholder 3 of 3: {endDate}                         [▲] [▼] ✕    │ ║  │
│  ║  ├─────────────────────────────────────────────────────────────────────┤ ║  │
│  ║  │  ● Simple Mode ○ Advanced Mode                                       │ ║  │
│  ║  │  Type: [date ▼]  Semantic: [date_range_end ▼]  Required: [ ]        │ ║  │
│  ║  │  Default: [GETUTCDATE()]  Validators: [valid-date ✕]                │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  [+ Add Another Placeholder]                                              ║  │
│  ║                                                                           ║  │
│  ║  ✅ Validation Summary                                                    ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  ✅ All SQL placeholders configured                                  │ ║  │
│  ║  │  ✅ No orphaned placeholders in SQL                                  │ ║  │
│  ║  │  ✅ 1 required field (patientId)                                     │ ║  │
│  ║  │  ✅ 2 optional fields with defaults                                  │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ╚═══════════════════════════════════════════════════════════════════════════╝  │
│                                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────────────┐│
│  │                       [← Back] [Save Draft] [Next: Examples →]              ││
│  └─────────────────────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────────────────┘
```

**Components Used:**

- `Accordion` or `Collapsible` for each placeholder card
- `RadioGroup` for Simple/Advanced mode toggle
- `Select` for type and semantic type dropdowns
- `Checkbox` for required, searchable, filterable
- `Input` for default values
- `Badge` for validator chips
- `Button` for preset actions and add/remove
- `Alert` for UI preview rendering

**Key Features:**

1. **Two-Mode Configuration:**

   - Simple Mode: Type + Required + Default + Validators (covers 80% of use cases)
   - Advanced Mode: Full schema integration with UI preview

2. **Quick Setup Presets:**

   - **Patient Filter**: Auto-configures `{patientId}` with schema linking
   - **Time Window**: Adds `{startDate}`, `{endDate}`, `{windowDays}` bundle
   - **Top-K Limiter**: Adds `{topK}` with min/max validators
   - **Copy from Template**: Clone placeholder configs from existing template

3. **Schema Integration:**

   - Links semantic types to database tables
   - Generates UI preview showing how placeholder will render in Apply Template wizard
   - Supports searchable dropdowns with display column expressions

4. **Validation:**
   - Checks all SQL placeholders are configured
   - Warns about placeholders in config but not in SQL (orphaned)
   - Validates data types match semantic types
   - Validator syntax validation

---

### Tab 4: Example Questions

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Create New Template                                                          ✕ │
├─────────────────────────────────────────────────────────────────────────────────┤
│  ┌─ Tabs ────────────────────────────────────────────────────────────────────┐  │
│  │  [1. Basic Info] [2. SQL Pattern] [3. Placeholders] [4. Examples] Preview │  │
│  └───────────────────────────────────────────────────────────────────────────┘  │
│                                                                                   │
│  ╔═══════════════════════════════════════════════════════════════════════════╗  │
│  ║  TAB 4: EXAMPLE QUESTIONS                                                 ║  │
│  ╠═══════════════════════════════════════════════════════════════════════════╣  │
│  ║                                                                           ║  │
│  ║  💡 Add 3-5 example questions to help template matching                    ║  │
│  ║  These examples train the AI to recognize when to use this template       ║  │
│  ║                                                                           ║  │
│  ║  Example 1 of 3                                                            ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │ How many assessments did this patient have in the last 6 months?    │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║  Matched Keywords: assessments, patient, 6 months                          ║  │
│  ║  [✕ Remove] [▲ Move Up] [▼ Move Down]                                     ║  │
│  ║                                                                           ║  │
│  ║  Example 2 of 3                                                            ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │ Count assessments for patient in past 180 days                      │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║  Matched Keywords: count, assessments, patient, 180 days                   ║  │
│  ║  [✕ Remove] [▲ Move Up] [▼ Move Down]                                     ║  │
│  ║                                                                           ║  │
│  ║  Example 3 of 3                                                            ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │ Show me assessment count by date range                               │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║  Matched Keywords: assessment, count, date range                           ║  │
│  ║  [✕ Remove] [▲ Move Up] [▼ Move Down]                                     ║  │
│  ║                                                                           ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │ + Add another example                                                │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  🎯 Smart Example Generator                                                ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  [🤖 Generate Examples from Keywords]                                │ ║  │
│  ║  │  [📋 Import from Similar Templates]                                  │ ║  │
│  ║  │  [📊 Use Most Common Questions]                                      │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  ⚡ Example Quality Check                                                  ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  ✅ 3 examples provided (minimum met)                                │ ║  │
│  ║  │  ✅ Examples cover all keywords                                      │ ║  │
│  ║  │  ✅ Examples show variety (different phrasings)                      │ ║  │
│  ║  │  💡 Tip: Add 2 more examples for better matching accuracy           │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ╚═══════════════════════════════════════════════════════════════════════════╝  │
│                                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────────────┐│
│  │                        [← Back] [Save Draft] [Preview & Finish →]           ││
│  └─────────────────────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────────────────┘
```

**Components Used:**

- `Textarea` for each example question
- `Button` for remove, move up/down actions
- `Badge` for keyword highlighting
- `Alert` for quality check feedback

**Smart Generators:**

- **Generate from Keywords**: Uses keywords to create natural language examples
- **Import from Similar**: Loads examples from templates with same intent
- **Most Common Questions**: Shows frequently asked questions from TemplateUsage logs

**Validation:**

- Minimum 3 examples required
- Each example should contain at least 2 keywords
- Examples should show variety (not duplicates with minor wording changes)

---

### Tab 5: Preview & Validation

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Create New Template                                                          ✕ │
├─────────────────────────────────────────────────────────────────────────────────┤
│  ┌─ Tabs ────────────────────────────────────────────────────────────────────┐  │
│  │  [1. Basic Info] [2. SQL Pattern] [3. Placeholders] [4. Examples] Preview │  │
│  └───────────────────────────────────────────────────────────────────────────┘  │
│                                                                                   │
│  ╔═══════════════════════════════════════════════════════════════════════════╗  │
│  ║  PREVIEW & VALIDATION                                                     ║  │
│  ╠═══════════════════════════════════════════════════════════════════════════╣  │
│  ║                                                                           ║  │
│  ║  📋 Template Summary                                                       ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  Name: Count Assessments by Time Window                             │ ║  │
│  ║  │  Intent: Aggregation by category                                    │ ║  │
│  ║  │  Keywords: count, assessments, time window, date range, patient     │ ║  │
│  ║  │  Tags: [assessment-analysis] [time-series]                          │ ║  │
│  ║  │  Status: Draft (will be saved as Draft)                             │ ║  │
│  ║  │                                                                      │ ║  │
│  ║  │  [Edit Basic Info →]                                                 │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  📝 SQL Pattern Preview                                                    ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  SELECT                                                              │ ║  │
│  ║  │      COUNT(DISTINCT A.id) AS assessmentCount                         │ ║  │
│  ║  │  FROM rpt.Assessment A                                               │ ║  │
│  ║  │  JOIN rpt.DimDate D ON A.dimDateFk = D.id                            │ ║  │
│  ║  │  WHERE                                                                │ ║  │
│  ║  │      A.patientFk = {patientId}                                       │ ║  │
│  ║  │      AND D.date > DATEADD(day, -{windowDays}, {endDate})            │ ║  │
│  ║  │      AND D.date <= {endDate}                                         │ ║  │
│  ║  │                                                                      │ ║  │
│  ║  │  [Edit SQL Pattern →]                                                │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  🎛️ Placeholders (3)                                                       ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  • {patientId} - guid, patient_id, required, searchable             │ ║  │
│  ║  │  • {windowDays} - int, time_window, optional (default: 180), 1-365  │ ║  │
│  ║  │  • {endDate} - date, date_range_end, optional (default: today)      │ ║  │
│  ║  │                                                                      │ ║  │
│  ║  │  [Edit Placeholders →]                                               │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  💬 Example Questions (3)                                                  ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  1. How many assessments did this patient have in the last 6 months?│ ║  │
│  ║  │  2. Count assessments for patient in past 180 days                  │ ║  │
│  ║  │  3. Show me assessment count by date range                           │ ║  │
│  ║  │                                                                      │ ║  │
│  ║  │  [Edit Examples →]                                                   │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  ✅ Final Validation                                                       ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  ✅ All required fields complete                                     │ ║  │
│  ║  │  ✅ SQL validation passed                                            │ ║  │
│  ║  │  ✅ All placeholders configured                                      │ ║  │
│  ║  │  ✅ Minimum 3 examples provided                                      │ ║  │
│  ║  │  ✅ Ready to save as Draft                                           │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  🎯 What Happens Next?                                                     ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  1. Template saved as Draft (version 1)                             │ ║  │
│  ║  │  2. Test it in the Funnel UI by asking matching questions           │ ║  │
│  ║  │  3. Edit if needed (Drafts are editable)                            │ ║  │
│  ║  │  4. Publish to make available to all developers                     │ ║  │
│  ║  │  5. Monitor usage and success rate in Template Admin                │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ╚═══════════════════════════════════════════════════════════════════════════╝  │
│                                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────────────┐│
│  │                        [← Back] [Save as Draft] [Save & Test in Funnel]     ││
│  └─────────────────────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────────────────┘
```

**Components Used:**

- `Card` or `Alert` for summary sections
- `Button` for edit actions linking back to specific tabs
- `Alert` (success variant) for validation results

**Actions:**

- **Edit [Section]**: Jumps to specific tab for edits
- **Save as Draft**: Creates Draft template, stays in modal to show confirmation
- **Save & Test in Funnel**: Creates Draft + navigates to Funnel page with template preselected

---

## Template Admin Page (Browse/Manage)

**Path:** `/admin/templates` or `/templates` (feature-flagged)

**Purpose:** Central hub for discovering, browsing, editing, and managing all templates

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                                                                                   │
│  Template Catalog                                             [+ New Template]   │
│                                                                                   │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                   │
│  ┌─ Search & Filters ────────────────────────────────────────────────────────┐  │
│  │                                                                            │  │
│  │  🔍 [Search templates by name, keywords, or description...___________]    │  │
│  │                                                                            │  │
│  │  Status: [All ▼] [Draft ▼] [Approved ▼] [Deprecated ▼]                   │  │
│  │  Intent: [All ▼] [Aggregation ▼] [Time-series ▼] [Top-k ▼] [Join ▼]     │  │
│  │  Tags:   [All ▼] [assessment-analysis ▼] [wound-tracking ▼] [patient ▼]  │  │
│  │  Sort by: [Most Used ▼] [Newest ▼] [Success Rate ▼] [Name A-Z ▼]         │  │
│  │                                                                            │  │
│  │  [Clear Filters]                                       23 templates found  │  │
│  │                                                                            │  │
│  └────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                   │
│  ┌─ Quick Stats ─────────────────────────────────────────────────────────────┐  │
│  │  📊 Total: 23  │  ✅ Approved: 18  │  📝 Draft: 3  │  🚫 Deprecated: 2   │  │
│  │  📈 Avg Success Rate: 82%  │  🔥 Most Used: "Count Assessments by Time"   │  │
│  └────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                   │
│  ┌─ View Mode ────────────────────────────────────────────────────────────────┐ │
│  │  [Grid View] [List View] [Table View]                      View: Grid     │ │
│  └────────────────────────────────────────────────────────────────────────────┘ │
│                                                                                   │
│  ┌───────────────────────────────────────────────────────────────────────────┐  │
│  │                           GRID VIEW (Default)                             │  │
│  ├───────────────────────────────────────────────────────────────────────────┤  │
│  │                                                                           │  │
│  │  ┌─────────────────────────┐  ┌─────────────────────────┐                │  │
│  │  │ 📋 Count Assessments by │  │ 📊 Wound State Timeline │                │  │
│  │  │    Time Window          │  │    by Patient           │                │  │
│  │  ├─────────────────────────┤  ├─────────────────────────┤                │  │
│  │  │                         │  │                         │                │  │
│  │  │ Status: ✅ Approved     │  │ Status: ✅ Approved     │                │  │
│  │  │ Intent: Aggregation     │  │ Intent: Time-series     │                │  │
│  │  │                         │  │                         │                │  │
│  │  │ Success Rate: 87%       │  │ Success Rate: 92%       │                │  │
│  │  │ Usage: 127 times        │  │ Usage: 89 times         │                │  │
│  │  │ Last used: 2 hours ago  │  │ Last used: 1 day ago    │                │  │
│  │  │                         │  │                         │                │  │
│  │  │ Tags:                   │  │ Tags:                   │                │  │
│  │  │ [assessment] [time]     │  │ [wound] [timeline]      │                │  │
│  │  │                         │  │                         │                │  │
│  │  │ Keywords: count,        │  │ Keywords: wound,        │                │  │
│  │  │ assessments, window     │  │ state, timeline         │                │  │
│  │  │                         │  │                         │                │  │
│  │  │ [View Details]          │  │ [View Details]          │                │  │
│  │  │ [Apply in Funnel]       │  │ [Apply in Funnel]       │                │  │
│  │  │ [⋮ More]                │  │ [⋮ More]                │                │  │
│  │  │   • Duplicate           │  │   • Duplicate           │                │  │
│  │  │   • Deprecate           │  │   • Deprecate           │                │  │
│  │  │   • View Usage          │  │   • View Usage          │                │  │
│  │  │   • Export JSON         │  │   • Export JSON         │                │  │
│  │  │                         │  │                         │                │  │
│  │  └─────────────────────────┘  └─────────────────────────┘                │  │
│  │                                                                           │  │
│  │  ┌─────────────────────────┐  ┌─────────────────────────┐                │  │
│  │  │ 📝 Patient Risk Score   │  │ 🔍 Top Wounds by        │                │  │
│  │  │    Calculation          │  │    Healing Time         │                │  │
│  │  ├─────────────────────────┤  ├─────────────────────────┤                │  │
│  │  │                         │  │                         │                │  │
│  │  │ Status: 📝 Draft        │  │ Status: ✅ Approved     │                │  │
│  │  │ Intent: Aggregation     │  │ Intent: Top-k           │                │  │
│  │  │                         │  │                         │                │  │
│  │  │ Success Rate: N/A       │  │ Success Rate: 78%       │                │  │
│  │  │ Usage: 0 times          │  │ Usage: 45 times         │                │  │
│  │  │ Created: 1 hour ago     │  │ Last used: 5 hours ago  │                │  │
│  │  │                         │  │                         │                │  │
│  │  │ Tags:                   │  │ Tags:                   │                │  │
│  │  │ [patient] [risk]        │  │ [wound] [top-k]         │                │  │
│  │  │                         │  │                         │                │  │
│  │  │ Keywords: patient,      │  │ Keywords: top, wounds,  │                │  │
│  │  │ risk, score             │  │ healing, time           │                │  │
│  │  │                         │  │                         │                │  │
│  │  │ [Edit Draft]            │  │ [View Details]          │                │  │
│  │  │ [Publish]               │  │ [Apply in Funnel]       │                │  │
│  │  │ [Delete Draft]          │  │ [⋮ More]                │                │  │
│  │  │                         │  │                         │                │  │
│  │  └─────────────────────────┘  └─────────────────────────┘                │  │
│  │                                                                           │  │
│  │  ┌─────────────────────────┐  ┌─────────────────────────┐                │  │
│  │  │ 🚫 Legacy Patient Count │  │ 📊 Assessment Frequency │                │  │
│  │  │    (DEPRECATED)         │  │    by Form Type         │                │  │
│  │  ├─────────────────────────┤  ├─────────────────────────┤                │  │
│  │  │                         │  │                         │                │  │
│  │  │ Status: 🚫 Deprecated   │  │ Status: ✅ Approved     │                │  │
│  │  │ Intent: Aggregation     │  │ Intent: Aggregation     │                │  │
│  │  │                         │  │                         │                │  │
│  │  │ Success Rate: 45% ⚠️    │  │ Success Rate: 95% 🔥    │                │  │
│  │  │ Usage: 89 (historic)    │  │ Usage: 203 times        │                │  │
│  │  │ Deprecated: 2 weeks ago │  │ Last used: 30 min ago   │                │  │
│  │  │                         │  │                         │                │  │
│  │  │ ⚠️ Use instead:          │  │ Tags:                   │                │  │
│  │  │ "Count Assessments v2"  │  │ [assessment] [frequency]│                │  │
│  │  │                         │  │                         │                │  │
│  │  │ [View History]          │  │ [View Details] ⭐       │                │  │
│  │  │ [View Replacement →]    │  │ [Apply in Funnel]       │                │  │
│  │  │                         │  │ [⋮ More]                │                │  │
│  │  │                         │  │                         │                │  │
│  │  └─────────────────────────┘  └─────────────────────────┘                │  │
│  │                                                                           │  │
│  │  [Load More...]                                            Page 1 of 4    │  │
│  │                                                                           │  │
│  └───────────────────────────────────────────────────────────────────────────┘  │
│                                                                                   │
└─────────────────────────────────────────────────────────────────────────────────┘
```

### Template Detail View (Modal or Side Panel)

**Opened when:** Click "View Details" on any template card

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Template Details: Count Assessments by Time Window                           ✕ │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                   │
│  ┌─ Header ──────────────────────────────────────────────────────────────────┐  │
│  │  Count Assessments by Time Window                    Status: ✅ Approved  │  │
│  │  Intent: Aggregation by category                     Version: 2 (active)  │  │
│  │                                                                            │  │
│  │  [Apply in Funnel] [Duplicate] [Deprecate] [⋮ More Actions]               │  │
│  └────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                   │
│  ┌─ Tabs ────────────────────────────────────────────────────────────────────┐  │
│  │  [Overview] [SQL Pattern] [Placeholders] [Examples] [Usage] [History]    │  │
│  └────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                   │
│  ╔═══════════════════════════════════════════════════════════════════════════╗  │
│  ║  TAB: OVERVIEW                                                            ║  │
│  ╠═══════════════════════════════════════════════════════════════════════════╣  │
│  ║                                                                           ║  │
│  ║  Description                                                              ║  │
│  ║  Counts distinct assessments within a specified time window for a        ║  │
│  ║  given patient. Useful for analyzing patient visit frequency and         ║  │
│  ║  engagement over custom time periods.                                     ║  │
│  ║                                                                           ║  │
│  ║  Keywords                                                                  ║  │
│  ║  [count] [assessments] [time window] [date range] [patient]               ║  │
│  ║                                                                           ║  │
│  ║  Tags                                                                      ║  │
│  ║  [assessment-analysis] [time-series]                                       ║  │
│  ║                                                                           ║  │
│  ║  📊 Usage Statistics                                                       ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  Total Uses: 127                      Last Used: 2 hours ago        │ ║  │
│  ║  │  Success Rate: 87% ✅                  Avg Latency: 450ms            │ ║  │
│  ║  │  Failed Uses: 16                       Error Types: 12 schema, 4 timeout│║║
│  ║  │                                                                      │ ║  │
│  ║  │  📈 Usage Trend (last 30 days)                                       │ ║  │
│  ║  │  [Mini line chart showing daily usage]                               │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  🔄 Version History                                                        ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  • Version 2 (active) - Published 3 weeks ago by alice@aranz.com    │ ║  │
│  ║  │    Changes: Added endDate parameter, improved date filtering        │ ║  │
│  ║  │    [View v2 Details] [Compare with v1]                               │ ║  │
│  ║  │                                                                      │ ║  │
│  ║  │  • Version 1 - Published 6 months ago by bob@aranz.com              │ ║  │
│  ║  │    Initial release                                                   │ ║  │
│  ║  │    [View v1 Details] [Restore to v1]                                 │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  📝 Metadata                                                               ║  │
│  ║  Created: 2024-04-15 by bob@aranz.com                                     ║  │
│  ║  Updated: 2024-09-20 by alice@aranz.com                                   ║  │
│  ║  Dialect: MS SQL Server                                                   ║  │
│  ║                                                                           ║  │
│  ╚═══════════════════════════════════════════════════════════════════════════╝  │
│                                                                                   │
│  [Close]                                                                          │
│                                                                                   │
└─────────────────────────────────────────────────────────────────────────────────┘
```

**Other Detail Tabs:**

- **SQL Pattern**: Read-only view of sqlPattern with syntax highlighting
- **Placeholders**: Table view of all placeholders with type/semantic/validators
- **Examples**: List of all example questions with matched keywords
- **Usage**: Detailed usage logs with success/failure breakdown, recent queries
- **History**: Full version changelog with diffs between versions

---

## Apply Template Wizard (Slot-Filling)

**Path:** Funnel Panel → "Apply Template" button (when template matched or manually selected)

**Purpose:** Fill placeholder values to generate SQL from template

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  Apply Template: Count Assessments by Time Window                            ✕ │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                   │
│  Fill in the required fields to generate SQL from this template                  │
│                                                                                   │
│  ╔═══════════════════════════════════════════════════════════════════════════╗  │
│  ║  REQUIRED FIELDS                                                          ║  │
│  ╠═══════════════════════════════════════════════════════════════════════════╣  │
│  ║                                                                           ║  │
│  ║  patientId * (guid)                                                        ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │ 🔍 Search patients...                                            ▼  │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║  💡 Start typing patient name or ID                                       ║  │
│  ║                                                                           ║  │
│  ║  Recent patients:                                                          ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  • John Smith (ID: ABC-123-XYZ) - Last accessed: 1 hour ago        │ ║  │
│  ║  │  • Jane Doe (ID: DEF-456-UVW) - Last accessed: 2 days ago          │ ║  │
│  ║  │  • Bob Martin (ID: GHI-789-RST) - Last accessed: 1 week ago        │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ╚═══════════════════════════════════════════════════════════════════════════╝  │
│                                                                                   │
│  ╔═══════════════════════════════════════════════════════════════════════════╗  │
│  ║  OPTIONAL FIELDS                                                          ║  │
│  ╠═══════════════════════════════════════════════════════════════════════════╣  │
│  ║                                                                           ║  │
│  ║  windowDays (int)                                                          ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │ 180                                                                  │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║  Default: 180  │  Range: 1-365                                             ║  │
│  ║  ℹ️ Number of days to look back                                            ║  │
│  ║                                                                           ║  │
│  ║  endDate (date)                                                            ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │ 2025-10-06                                                      📅  │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║  Default: Today                                                            ║  │
│  ║  ℹ️ End date for the time window                                           ║  │
│  ║                                                                           ║  │
│  ╚═══════════════════════════════════════════════════════════════════════════╝  │
│                                                                                   │
│  ╔═══════════════════════════════════════════════════════════════════════════╗  │
│  ║  SQL PREVIEW                                                              ║  │
│  ╠═══════════════════════════════════════════════════════════════════════════╣  │
│  ║                                                                           ║  │
│  ║  ┌─────────────────────────────────────────────────────────────────────┐ ║  │
│  ║  │  SELECT                                                              │ ║  │
│  ║  │      COUNT(DISTINCT A.id) AS assessmentCount                         │ ║  │
│  ║  │  FROM rpt.Assessment A                                               │ ║  │
│  ║  │  JOIN rpt.DimDate D ON A.dimDateFk = D.id                            │ ║  │
│  ║  │  WHERE                                                                │ ║  │
│  ║  │      A.patientFk = 'ABC-123-XYZ'                                     │ ║  │
│  ║  │      AND D.date > DATEADD(day, -180, '2025-10-06')                  │ ║  │
│  ║  │      AND D.date <= '2025-10-06'                                      │ ║  │
│  ║  └─────────────────────────────────────────────────────────────────────┘ ║  │
│  ║                                                                           ║  │
│  ║  ✅ Validation passed - Ready to generate                                 ║  │
│  ║                                                                           ║  │
│  ╚═══════════════════════════════════════════════════════════════════════════╝  │
│                                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────────────┐│
│  │                               [Cancel] [Generate SQL & Execute]             ││
│  └─────────────────────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────────────────┘
```

**Components Used:**

- `Dialog` (medium size)
- `Select` with `Combobox` for searchable patient dropdown
- `Input` (number) for windowDays with validation
- `Input` (date) or `DatePicker` for endDate
- `Textarea` (read-only, monospace) for SQL preview
- `Alert` for validation status

**Features:**

- **Real-time Preview**: SQL updates as user fills fields
- **Schema-powered Dropdowns**: Patient selector queries rpt.Patient with search
- **Validation**: Required fields checked; validators (min/max) enforced
- **Smart Defaults**: Optional fields pre-filled with defaults from placeholdersSpec
- **Recent Values**: Shows recently used patients for quick selection

---

## Funnel Panel Template Info

**Path:** Funnel Panel → After SQL generated with matched template

**Purpose:** Show which template was used and allow applying templates

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  FunnelPanel (existing component with template info added)                       │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                   │
│  [... existing Question and SQL sections ...]                                    │
│                                                                                   │
│  ┌─ SQL Metadata (existing expandable sections) ────────────────────────────┐   │
│  │                                                                            │   │
│  │  📝 Explanation  [▼]                                                       │   │
│  │  🔍 Validation Notes  [▼]                                                  │   │
│  │                                                                            │   │
│  │  🏷️ Matched Template  [▼]                                                  │   │
│  │  ┌────────────────────────────────────────────────────────────────────┐   │   │
│  │  │  Template: Count Assessments by Time Window                        │   │   │
│  │  │  Match Score: 0.87  │  Success Rate: 87%                           │   │   │
│  │  │                                                                     │   │   │
│  │  │  💡 Why matched:                                                    │   │   │
│  │  │  • Keywords: "count", "assessments", "time window"                 │   │   │
│  │  │  • Example: "How many assessments in the last 6 months"            │   │   │
│  │  │  • Intent: Aggregation by category                                 │   │   │
│  │  │                                                                     │   │   │
│  │  │  [View Template Details] [Apply Different Template] [Save as New]  │   │   │
│  │  └────────────────────────────────────────────────────────────────────┘   │   │
│  │                                                                            │   │
│  └────────────────────────────────────────────────────────────────────────────┘   │
│                                                                                   │
│  ┌─ Template Actions ─────────────────────────────────────────────────────────┐  │
│  │  💡 This SQL works well! Turn it into a reusable template:                │  │
│  │  [💾 Save as Template]                                                     │  │
│  └────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                   │
└─────────────────────────────────────────────────────────────────────────────────┘
```

**Components Used:**

- `Collapsible` for "Matched Template" section (consistent with existing Explanation/Validation Notes)
- `Badge` for match score and success rate
- `Alert` (info variant) for "Save as Template" prompt
- `Button` for template actions

**Features:**

- **Match Rationale**: Shows why this template was selected (keywords, example, score)
- **Success Rate**: Displays template's historical success rate from TemplateUsage
- **Actions**:
  - **View Template Details**: Opens Template Detail modal
  - **Apply Different Template**: Opens template selector to manually pick another template
  - **Save as New**: Creates new template from current SQL (if no template matched or user customized)

---

## Field Mapping Reference

### User-Entered Fields (Template Editor)

| Field                | Input Type   | Location         | Required | Notes                               |
| -------------------- | ------------ | ---------------- | -------- | ----------------------------------- |
| **name**             | Text         | Tab 1            | ✅       | 3-100 chars, descriptive            |
| **intent**           | Dropdown     | Tab 1            | ✅       | From taxonomy (7 options)           |
| **description**      | Textarea     | Tab 1            | ❌       | 100-300 chars recommended           |
| **keywords**         | Tag input    | Tab 1            | ✅       | Comma-separated, min 3              |
| **tags**             | Tag picker   | Tab 1            | ❌       | For organization/filtering          |
| **dialect**          | Dropdown     | Tab 1            | ✅       | Default 'mssql'                     |
| **sqlPattern**       | Code editor  | Tab 2            | ✅       | Must contain SELECT; {placeholders} |
| **placeholdersSpec** | Complex form | Tab 3            | ✅       | See placeholder table below         |
| **examples**         | Text list    | Tab 4            | ✅       | Min 3, max recommended 5            |
| **notes**            | Textarea     | Tab 1 (optional) | ❌       | Developer notes                     |

### Placeholder Configuration Fields (per placeholder)

| Field                             | Input Type           | Required | Notes                                                |
| --------------------------------- | -------------------- | -------- | ---------------------------------------------------- |
| **name**                          | Text (auto-detected) | ✅       | From {placeholder} in SQL                            |
| **type**                          | Dropdown             | ✅       | guid, int, string, date, boolean, decimal            |
| **semantic**                      | Dropdown             | ✅       | patient_id, wound_id, time_window, etc.              |
| **required**                      | Checkbox             | ✅       | Must fill when applying template                     |
| **default**                       | Text                 | ❌       | Can be SQL expression (e.g., GETUTCDATE())           |
| **validators**                    | Multi-tag            | ❌       | non-empty, min:N, max:N, regex:pattern               |
| **Schema Integration (Advanced)** |                      |          |                                                      |
| **sourceTable**                   | Dropdown             | ❌       | rpt.Patient, rpt.Wound, etc.                         |
| **valueColumn**                   | Dropdown             | ❌       | Column to use as value (usually 'id')                |
| **displayColumn**                 | Expression           | ❌       | Column(s) to show in UI (e.g., firstName + lastName) |
| **searchable**                    | Checkbox             | ❌       | Enable search in Apply Template wizard               |
| **filterable**                    | Checkbox             | ❌       | Enable filtering in Apply Template wizard            |

### Auto-Generated Fields

| Field                               | Source                   | Notes                                |
| ----------------------------------- | ------------------------ | ------------------------------------ |
| **Template.id**                     | Database serial          |                                      |
| **Template.status**                 | 'Draft' on create        | Changed to 'Approved' on publish     |
| **Template.activeVersionId**        | Set on publish           | Points to current TemplateVersion.id |
| **Template.createdBy**              | Auth session             | User email/username                  |
| **Template.createdAt**              | NOW()                    |                                      |
| **Template.updatedAt**              | Trigger                  | Auto-updates on any change           |
| **TemplateVersion.id**              | Database serial          |                                      |
| **TemplateVersion.templateId**      | FK linkage               |                                      |
| **TemplateVersion.version**         | Increment                | 1, 2, 3... (immutable)               |
| **TemplateVersion.validationRules** | Generated from validator | JSON of validation results           |
| **TemplateVersion.resultShape**     | Inferred from SELECT     | Phase 2 feature                      |
| **TemplateVersion.createdAt**       | NOW()                    |                                      |

---

## Component Usage Guidelines

### shadcn/ui Components Mapping

| UI Element           | Component                                                 | Import                        |
| -------------------- | --------------------------------------------------------- | ----------------------------- |
| Modal dialogs        | `Dialog`, `DialogContent`, `DialogHeader`, `DialogFooter` | `@/components/ui/dialog`      |
| Tabs                 | `Tabs`, `TabsList`, `TabsTrigger`, `TabsContent`          | `@/components/ui/tabs`        |
| Text inputs          | `Input`                                                   | `@/components/ui/input`       |
| Dropdowns            | `Select`, `SelectTrigger`, `SelectContent`, `SelectItem`  | `@/components/ui/select`      |
| Searchable dropdowns | `Combobox` (or custom with `Popover` + `Command`)         | `@/components/ui/command`     |
| Multi-line text      | `Textarea`                                                | `@/components/ui/textarea`    |
| Tags/chips           | `Badge`                                                   | `@/components/ui/badge`       |
| Buttons              | `Button`                                                  | `@/components/ui/button`      |
| Alerts/notifications | `Alert`, `AlertTitle`, `AlertDescription`                 | `@/components/ui/alert`       |
| Expandable sections  | `Collapsible`, `CollapsibleTrigger`, `CollapsibleContent` | `@/components/ui/collapsible` |
| Cards                | `Card`, `CardHeader`, `CardContent`, `CardFooter`         | `@/components/ui/card`        |
| Toggle groups        | `RadioGroup`, `RadioGroupItem`                            | `@/components/ui/radio-group` |
| Checkboxes           | `Checkbox`                                                | `@/components/ui/checkbox`    |
| Date picker          | `Calendar` (Radix)                                        | `@/components/ui/calendar`    |

### Styling Conventions

**Consistent with existing Funnel UI:**

- Use Tailwind utility classes
- Color scheme:
  - Primary actions: `bg-blue-500`, `text-white`, `hover:bg-blue-600`
  - Success: `bg-green-500`, `text-white`, `hover:bg-green-600`
  - Warning: `bg-yellow-500`, `text-white`, `hover:bg-yellow-600`
  - Danger: `bg-red-500`, `text-white`, `hover:bg-red-600`
  - Draft status: `bg-yellow-100`, `text-yellow-800`
  - Approved status: `bg-green-100`, `text-green-800`
  - Deprecated status: `bg-red-100`, `text-red-800`
- Spacing: Consistent padding (p-4, p-6) and gaps (space-y-4, gap-4)
- Borders: `border`, `border-gray-200`, `rounded-lg`, `shadow-md`
- Text sizes:
  - Headers: `text-lg`, `font-semibold`
  - Labels: `text-sm`, `font-medium`
  - Body: `text-sm`
  - Helper text: `text-xs`, `text-gray-500`

### Validation Feedback

**Real-time validation display:**

- ✅ Success: Green checkmark + message
- ⚠️ Warning: Yellow warning icon + suggestion
- ❌ Error: Red X + error message with actionable fix

**Example:**

```tsx
<Alert variant="destructive"> {/* Error */}
  <AlertTriangle className="h-4 w-4" />
  <AlertTitle>Validation Error</AlertTitle>
  <AlertDescription>
    Placeholder {patientId} is used in SQL but not configured. Go to Tab 3 to configure it.
  </AlertDescription>
</Alert>

<Alert> {/* Info/Success */}
  <CheckCircle2 className="h-4 w-4" />
  <AlertTitle>All checks passed!</AlertTitle>
  <AlertDescription>
    Template is ready to save as Draft.
  </AlertDescription>
</Alert>
```

---

## Responsive Design

**Desktop (Primary):**

- Modal: Max-width 1200px for Template Editor
- Grid view: 3 columns for template cards

**Tablet (768px - 1024px):**

- Modal: Max-width 90vw
- Grid view: 2 columns

**Mobile (< 768px):**

- Modal: Full-screen
- Grid view: 1 column (stacked)
- Tabs: Horizontal scrollable tabs
- Placeholder cards: Collapse to simple mode by default

---

## Accessibility

- All form inputs have associated `<label>` elements
- Keyboard navigation: Tab through fields, Enter to submit, Esc to close modals
- ARIA labels for icon-only buttons
- Focus indicators visible on all interactive elements
- Error messages linked to form fields with `aria-describedby`
- Color contrast meets WCAG AA standards (4.5:1 for text)

---

## Future Enhancements (Phase 2)

**Not in MVP but planned:**

- Drag-and-drop placeholder reordering
- SQL syntax highlighting with Monaco Editor
- Template diff viewer (compare versions side-by-side)
- Bulk import/export templates as JSON bundles
- Template performance profiling dashboard
- A/B testing UI for template variations
- AI-generated example questions from SQL pattern
- Smart placeholder type inference from SQL usage patterns

---

## Related Documents

- [Template Improvement Design](./template_improvement_design.md) - Architecture and data model
- [Template System MVP Implementation Plan](../todos/template_system_mvp_implementation_plan.md) - Development stages
- [Database Schema](../../database/migration/011_create_template_catalog.sql) - Table definitions

---

**End of UI Mockups Document**
