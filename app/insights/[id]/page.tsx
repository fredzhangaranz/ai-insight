...
export default function InsightDetailPage({ params }: { params: { id: string } }) {
  ...
  const [execData, setExecData] = useState<{ rows: any[]; chart: { chartType: ChartType; data: any } } | null>(null);
  ...
  const execute = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/insights/${params.id}/execute`, { method: "POST" });
  ...
  if (!enabled) return <div className="p-6 text-sm text-gray-600">Insights are disabled.</div>;
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex gap-2">
        <button onClick={execute} className="px-3 py-2 bg-blue-600 text-white rounded disabled:opacity-50" disabled={execLoading}>
      ...
      {execData && (
        <div className="space-y-3">
          <div className="text-sm text-gray-700">Rows returned: {execData.rows?.length || 0}</div>
          <div className="h-96 bg-white border rounded p-4">
            <ChartComponent chartType={execData.chart.chartType} data={execData.chart.data} title={insight.name} className="w-full h-full" />
          </div>
        </div>
      )}
    </div>
