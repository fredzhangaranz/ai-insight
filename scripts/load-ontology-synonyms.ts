...
async function loadOntologySynonyms() {
  ...
  try {
    ...
    const data = yaml.load(yamlContent) as OntologyData;
    ...
    console.log(
      '  SELECT preferred_term, category, array_length(synonyms, 1) as synonym_count FROM "ClinicalOntology" WHERE synonyms IS NOT NULL;'
    );
    console.log(
